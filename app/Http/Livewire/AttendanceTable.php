...
final class AttendanceTable extends PowerGridComponent
{
    ...
    public function bulkCheckedDelete()
    {
        if (auth()->check()) {
            ...
            try {
                ...
                $this->dispatchBrowserEvent('showToast', ['success' => true, 'message' => 'Data presensi berhasi dihapus.']);
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Batas', 'limit'),

            Column::make('Dibuat pada Tanggal', 'created_at')
                ->hidden(),

            Column::make('Dibuat pada Tanggal', 'created_at_formatted', 'created_at')
    ...
    */
    ...
    public function checkEmployeePresence()
{
    // Perform verification to check if the employee is physically present
    // This could involve GPS location, IP verification, or any other method

    // For example, using a simulated verification with a checkbox
    // You can replace this with a real verification method
    $this->isPresent = true;
}
...
{
    if ($this->isPresent) {
        // Perform the action when the employee is physically present
        // For example, record the attendance or perform other necessary actions

        // Reset the isPresent status after the action
        $this->isPresent = false;

        // Additional logic for handling the 'Present' action
        // ...
    } else {
        // Show an error message or prevent the action
        // For example:
        session()->flash('error', 'You must confirm your physical presence before marking as present.');
    }
}
