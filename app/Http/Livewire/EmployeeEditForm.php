...
class EmployeeEditForm extends Component
{
    ...
    public function saveEmployees()
    {
        ...
        $this->validate([
            ...
            'employees.*.phone' => 'required|min:11|max:13|',            
        ...
        foreach ($this->employees as $employee) {
            ...
            if (!$this->isUniqueOnDatabase($employeeBeforeUpdated, $employee, 'phone', User::class)) {
                $this->dispatchBrowserEvent('livewire-scroll', ['top' => 0]);
                return session()->flash('failed', "No. Telp dari data anggota {$employee['id']} sudah terdaftar. Silahkan masukan email yang berbeda!");
            }

            if (!$this->isUniqueOnDatabase($employeeBeforeUpdated, $employee, 'email', User::class)) {
                $this->dispatchBrowserEvent('livewire-scroll', ['top' => 0]);
                return session()->flash('failed', "Email dari data anggota {$employee['id']} sudah terdaftar. Silahkan masukan email yang berbeda!");
            }
        ...
        $message = $affected === 0 ?
            "Tidak ada data anggota yang diubah." :
            "Ada $affected data anggota yang berhasil diedit.";
