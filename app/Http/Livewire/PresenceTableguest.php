...
final class PresenceTableGuest extends PowerGridComponent
{
    use ActionButton;
    ...
    */
    ...
    {
        $presence = Presence::where('guest', 1)
            ->where('attendance_id', $this->attendanceId)
            ->first(); // Ambil satu baris pertama atau null jika tidak ditemukan

        if ($presence) {
            return Presence::query()
                ->where('attendance_id', $this->attendanceId)
                ->join('guests', 'presences.user_id', '=', 'guests.id')
                ->select('presences.*', 'guests.name as user_name');
        } else {
            return Presence::query()->where('attendance_id', 0)->where('user_id', 0)->where('id', 0);
        }
    }
    ...
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn("is_permission", fn (Presence $model) => $model->is_permission ?
                '<span class="btn btn-sm btn-warning d-block mx-auto text-center">Izin</span>' : '<span class="btn btn-sm btn-success d-block mx-auto text-center">Hadir</span>')
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Dibuat pada', 'created_at_formatted')
